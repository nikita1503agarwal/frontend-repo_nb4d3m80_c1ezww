      ...
      <div className="max-w-6xl mx-auto px-4 py-10 grid sm:grid-cols-2 md:grid-cols-4 gap-8 text-slate-300">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded bg-gradient-to-tr from-amber-400 to-red-500 grid place-items-center">
              <span className="text-white font-black text-sm">★</span>
            </div>
            <p className="text-white font-semibold">Star Kebab & Pizza</p>
          </div>
          <p className="text-sm">Family-owned kebab and pizza in Vienna. Fresh ingredients, fast service, big portions.</p>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <a href="tel:+43600123456" className="px-3 py-2 rounded-md bg-emerald-500 text-white text-sm hover:bg-emerald-600">Call Us</a>
            <a href="https://maps.google.com/?q=Star+Kebab+%26+Pizza+Vienna" className="px-3 py-2 rounded-md bg-indigo-500 text-white text-sm hover:bg-indigo-600" target="_blank" rel="noreferrer">Get Directions</a>
          </div>
        </div>
      </div>
      <div className="border-t border-white/10 py-4 text-center text-xs text-slate-400">© {new Date().getFullYear()} Star Kebab & Pizza • Vienna</div>
    </footer>
  )
}
